...
def main():
    ...
    for model_name, adapter in models:
        ...
        print(f"Riduzione: {result.token_reduction} token ({result.metadata['reduction_percentage']:.1%})")
    ...
    # Header
    print(f"{'Modello':<20} {'Token':<10} {'Riduz%':<10} {'Costo Pre':<12} {'Costo Post':<12} {'Risparmio':<12}")
    print("-" * 80)
    ...
    for r in results:
        ...
        print(f"{model_name:<20} {tokens_str:<10} {reduction_pct:>8.1%} {cost_before_str:<12} {cost_after_str:<12} {savings_str:<12}")
