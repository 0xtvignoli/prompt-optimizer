...
class OpenAIAdapter(LLMAdapter):
    ...
    def __init__(
        self, model_name: str = "gpt-3.5-turbo", config: Optional[ModelConfig] = None
    ):
        """
    ...
    def _estimate_gpt_tokens(self, text: str) -> int:
        ...
        total_estimate = (
            base_estimate + long_word_bonus + punctuation_bonus + special_bonus
        )
    ...
    def _optimize_instruction_format(self, prompt: str) -> str:
        ...
        optimized = re.sub(
            r"\bI would like you to\b", "", optimized, flags=re.IGNORECASE
        )

        # Optimizes for formato chat if necessario
        if self.model_name.startswith("gpt-3.5-turbo") or self.model_name.startswith(
            "gpt-4"
        ):
    ...
    def _optimize_for_chat_format(self, prompt: str) -> str:
        ...
        chat_optimized = re.sub(
            r"\b(thank you|thanks)\b.*?[.!]\s*", "", prompt, flags=re.IGNORECASE
        )
        ...
        for pattern, replacement in replacements.items():
            chat_optimized = re.sub(
                pattern, replacement, chat_optimized, flags=re.IGNORECASE
            )
