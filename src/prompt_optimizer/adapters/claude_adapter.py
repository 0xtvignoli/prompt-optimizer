...
class ClaudeAdapter(LLMAdapter):
    ...
    def __init__(
        self, model_name: str = "claude-3-sonnet", config: Optional[ModelConfig] = None
    ):
        """
    ...
    def _add_structural_tags_if_needed(self, prompt: str) -> str:
        ...
        if self.config.custom_params and self.config.custom_params.get(
            "use_xml_tags", False
        ):
            structured_sections = []
            ...
            for i, section in enumerate(sections):
                ...
                if section:
                    ...
                    if i == 0 or any(
                        kw in section.lower() for kw in ["context", "background"]
                    ):
    ...
    def _optimize_examples_for_claude(self, prompt: str) -> str:
        ...
        example_pattern = (
            r"(example|example|e\.g\.|for instance)[\s:]+(.*?)(?=\n\n|\n[A-Z]|$)"
        )
        ...
        optimized = re.sub(
            example_pattern, format_example, prompt, flags=re.IGNORECASE | re.DOTALL
        )
