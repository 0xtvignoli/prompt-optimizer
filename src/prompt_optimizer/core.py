...
class PromptOptimizer:
            ...
            if self._should_apply_strategy(strategy, current_prompt, target_reduction):
                try:
                    ...
                    if self._validate_optimization(prompt, optimized):
                        ...
                        self.logger.debug(
                            f"Applicata strategy: {strategy.__class__.__name__}"
                        )

                except Exception as e:
                    self.logger.warning(
                        f"Error applicando strategy {strategy.__class__.__name__}: {e}"
                    )

        # Calculates metriche finali
        optimized_tokens = self._count_tokens(current_prompt)
        token_reduction = original_tokens - optimized_tokens
        semantic_similarity = self.semantic_metrics.calculate_similarity(
        ...
        result = OptimizationResult(
            ...
            metadata={
                ...
                "reduction_percentage": (
                    token_reduction / original_tokens if original_tokens > 0 else 0
                ),
            },
        )

        self.logger.info(
            f"Ottimizzazione completata: {token_reduction} tokens saved "
            f"({result.metadata['reduction_percentage']:.2%})"
        )

        return result

    def batch_optimize(self, prompts: List[str], **kwargs) -> List[OptimizationResult]:
        """
    ...
    def _select_strategies(
        self, custom_strategies: Optional[List[str]]
    ) -> List[OptimizationStrategy]:
        """Seleziona le strategie from applicare."""
        if custom_strategies:
            return [
                s for s in self.strategies if s.__class__.__name__ in custom_strategies
            ]
